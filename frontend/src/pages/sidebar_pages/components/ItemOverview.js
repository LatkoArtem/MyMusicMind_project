  ...
  const fetchArtistBio = async (artistName) => {
    try {
      const response = await fetch(
        `https://mymusicmind-9gke.onrender.com/artist_info/name/${encodeURIComponent(artistName)}`,
        {
          credentials: "include",
        }
      );
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const releaseRes = await fetch(
          `https://mymusicmind-9gke.onrender.com/artist_latest_release/${encodeURIComponent(title)}`,
          {
            credentials: "include",
          }
        );
      ...
      try {
        ...
        const genreRes = await fetch(
          `https://mymusicmind-9gke.onrender.com/genre_evolution/${encodeURIComponent(title)}`,
          {
            credentials: "include",
          }
        );
