...
const MyMusicTrends = () => {
  ...
  useEffect(() => {
    const checkLogin = async () => {
      try {
        const res = await fetch("https://mymusicmind-backend.onrender.com/profile", { credentials: "include" });
        if (res.status === 200) {
    ...
    const fetchTop = async () => {
      ...
      try {
        const [artistsRes, tracksRes] = await Promise.all([
          fetch(`https://mymusicmind-backend.onrender.com/spotify/top-artists?time_range=${period}&limit=20`, {
            credentials: "include",
          }),
          fetch(`https://mymusicmind-backend.onrender.com/spotify/top-tracks?time_range=${period}&limit=50`, {
            credentials: "include",
          }),
        ]);
