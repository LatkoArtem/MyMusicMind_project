...
export default function RateTrackOrAlbum() {
  ...
  const handleSubmitRating = async () => {
    ...
    if (existingRating) {
      await fetch(`https://mymusicmind-9gke.onrender.com/api/ratings/${existingRating.id}`, {
        method: "PATCH",
      ...
      await fetch("https://mymusicmind-9gke.onrender.com/api/ratings", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const resp = await fetch(`https://mymusicmind-9gke.onrender.com/api/${type}/${id}`, {
          credentials: "include",
        });
        if (!resp.ok) return;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const resp = await fetch(`https://mymusicmind-9gke.onrender.com/api/ratings`, {
          credentials: "include",
        });
        if (!resp.ok) return;
    ...
    if (!id && query.trim() !== "") {
      const timeout = setTimeout(async () => {
        const resp = await fetch(
          `https://mymusicmind-9gke.onrender.com/api/search?type=${type || "track"}&query=${query}`,
          {
            credentials: "include",
          }
        );
